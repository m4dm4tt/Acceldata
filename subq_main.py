...
__assumptions__ = "The SQL construct is well formed. E.g., An SQL query should always end with a ';' or '/'"
...
def find_clauses_start_end_pos():
	...
	write_debug_out("Between clause pairs: ")
	for l_index,l_token in enumerate(g_all_tokens):
	...
	write_debug_out("Order by clause pairs: ")
	for l_index,l_token in enumerate(g_all_tokens):
	...
	l_join_clause_pairs = []
	write_debug_out("Join clause pairs: ")
	for l_index,l_token in enumerate(g_all_tokens):
		if (str(l_token).lower() in subq_lib.joins_tokens_array) :
			l_join_pos = l_index
			print("\t"+"JOIN TYPE: "+l_token)
			print(l_join_pos)
					

	return(l_between_clause_pairs, l_order_by_clause_pairs)

def find_join_clauses(l_start_pos, l_end_pos):
	l_end_pos = len(g_all_tokens)
	write_debug_out("JOIN clauses: ")
	for l_index,l_token in enumerate(g_all_tokens):
		if str(g_all_tokens[l_index]).lower() == 'where':
			l_between_pos = l_index
			for k in range(l_between_pos+1, l_end_pos):
				if str(g_all_tokens[k]).lower() == 'and':
					l_separator_and_pos = k
...
def print_table_list(l_start_pos, l_end_pos):
	...
	print('\t'+ "TABLE list along with ALIAS: " )
	...
	for j in range(l_from_pos+1, l_end_pos):
		if (str(g_all_tokens[j]).lower() == 'where') or (str(g_all_tokens[j]).lower() == 'on') (l_from_pos+1 == l_end_pos):
			break
		else:
...
if __name__== "__main__":
	...
	g_between_clause_pairs, g_order_by_clause_pairs =  find_clauses_start_end_pos()
#	print_subqs()
#	print_limit_clause()
